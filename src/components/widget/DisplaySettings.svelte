...
import type { FontSize, LayoutDensity } from "@utils/setting-utils";
import {
	getCardRadius,
	getDefaultHue,
	getFontSize,
...
let bgBlur = Number.parseInt(localStorage.getItem("bg-blur") || "0", 10);
let cardOpacity = Number.parseFloat(
	localStorage.getItem("card-opacity") || "1",
);
let cardBlur = Number.parseInt(localStorage.getItem("card-blur") || "0", 10);
let pageSize = Number.parseInt(localStorage.getItem("page-size") || "8", 10);

function applyCardStyle() {
	let el = document.getElementById("settings-card-style") as HTMLStyleElement;
	if (!el) {
		el = document.createElement("style");
		el.id = "settings-card-style";
...
function setPageSize(v: number) {
	pageSize = v;
	localStorage.setItem("page-size", String(v));
...
function render() {
	...
	box.style.cssText =
		"background:var(--float-panel-bg);border-radius:var(--radius-large);width:440px;max-width:calc(100vw - 32px);max-height:calc(100vh - 40px);overflow-y:auto;padding:24px;box-shadow:0 8px 40px rgba(0,0,0,0.15)";
	...
	// Header
	box.appendChild(
		S(`<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:20px">
	...
	box.appendChild(
		rangeSlider(
			...
			5,
			"",
			"linear-gradient(to right,oklch(0.80 0.10 0),oklch(0.80 0.10 60),oklch(0.80 0.10 120),oklch(0.80 0.10 180),oklch(0.80 0.10 240),oklch(0.80 0.10 300),oklch(0.80 0.10 360))",
		),
	);
	...
	box.appendChild(
		rangeSlider("radius", cardRadius, 0, 24, 1, "px", "oklch(0.80 0.05 250)"),
	);
	box.appendChild(
		rangeSlider(
			"opacity",
	...
	box.appendChild(
		rangeSlider("cardblur", cardBlur, 0, 30, 1, "px", "rgba(128,128,128,0.3)"),
	);
	...
	box.appendChild(
		choiceBtns(
			...
			["紧凑", "标准", "舒适"],
			(v) => {
				layoutDensity = v as LayoutDensity;
				setLayoutDensity(v as LayoutDensity);
				render();
			},
		),
	);
	...
	box.appendChild(
		rangeSlider("bgblur", bgBlur, 0, 40, 1, "px", "rgba(128,128,128,0.3)"),
	);

	// Pagination
	box.appendChild(sectionHdr("翻页"));
	box.appendChild(
		rangeSlider("pagesize", pageSize, 3, 30, 1, "", "rgba(128,128,128,0.3)"),
	);
			...
			if (valSpan)
				valSpan.textContent =
					input.dataset.suffix === "%"
						? `${v}%`
						: input.dataset.suffix
							? v + input.dataset.suffix
							: String(v);
			onChange(v);
		});
		// Store suffix on the input for reference
		input.dataset.suffix =
			input.closest("[data-suffix]")?.getAttribute("data-suffix") || "";
	}
	...
	bindSlider2("radius", (v) => {
		cardRadius = v;
		setCardRadius(v);
	});
	bindSlider2("opacity", (v) => {
		cardOpacity = v / 100;
		applyCardStyle();
	});
	bindSlider2("cardblur", (v) => {
		cardBlur = v;
		applyCardStyle();
	});
	bindSlider2("bgblur", (v) => {
		bgBlur = v;
		applyBg();
	});
	bindSlider2("pagesize", (v) => {
		setPageSize(v);
	});
...
const IB =
	"width:28px;height:28px;border:none;border-radius:6px;background:var(--btn-regular-bg);color:var(--btn-content);cursor:pointer;display:flex;align-items:center;justify-content:center;font-size:14px;flex-shrink:0";

function sectionHdr(label: string): HTMLDivElement {
	const d = document.createElement("div");
	d.style.cssText =
		"font-size:12px;font-weight:700;color:var(--meta-divider);text-transform:uppercase;letter-spacing:0.5px;margin:16px 0 8px;padding-bottom:4px;border-bottom:1px solid var(--line-divider)";
	d.textContent = label;
	return d;
}
