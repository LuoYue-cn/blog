...
import { verifyToken, saveToken, clearToken, hasToken, fetchPublicData, saveData, fetchData } from '@api/github';
import { setProfile, setAboutText, setSkills, setHue, setCardRadius, setLayoutDensity, setFontSize } from '@utils/setting-utils';
...
onMount(() => {
  ...
  window.addEventListener('open-login', () => { show = true; render(); });
...
async function handleLogin() {
  ...
  if (!t) { error = '请输入 Token'; return; }
  loading = true; error = '';
  try {
    const u = await verifyToken(t);
    if (!u) { error = 'Token 无效，请检查'; loading = false; return; }
    saveToken(t);
    user = u;
    show = false;
...
async function handlePush() {
  ...
  try {
    ...
    try { const existing = await fetchData(); if (existing) sha = existing.sha; } catch {}
    await saveData(data, sha);
    error = '';
    render();
    // Show temporary success
    const btn = document.getElementById('push-btn');
    if (btn) { btn.textContent = '✅ 已推送'; setTimeout(() => render(), 2000); }
  } catch(e: any) {
...
async function handlePull() {
  ...
  try {
    ...
    if (!data) { error = 'GitHub 上没有数据'; loading = false; render(); return; }
    applyDataToLocal(data);
    error = '';
    window.dispatchEvent(new CustomEvent('profile-updated'));
    render();
    const btn = document.getElementById('pull-btn');
    if (btn) { btn.textContent = '✅ 已同步'; setTimeout(() => render(), 2000); }
  } catch(e: any) {
...
function close() { show = false; render(); }
...
function applyDataToLocal(data: any) {
  ...
  if (data.settings) {
    ...
    if (s.hue) { localStorage.setItem('hue', String(s.hue)); setHue(Number(s.hue)); }
    if (s['card-radius']) { localStorage.setItem('card-radius', String(s['card-radius'])); setCardRadius(Number(s['card-radius'])); }
    if (s['layout-density']) { localStorage.setItem('layout-density', s['layout-density']); setLayoutDensity(s['layout-density']); }
    if (s['font-size']) { localStorage.setItem('font-size', s['font-size']); setFontSize(s['font-size']); }
    if (s['bg-url']) localStorage.setItem('bg-url', s['bg-url']);
    if (s['bg-blur']) localStorage.setItem('bg-blur', String(s['bg-blur']));
    if (s['card-opacity']) localStorage.setItem('card-opacity', String(s['card-opacity']));
    if (s['card-blur']) localStorage.setItem('card-blur', String(s['card-blur']));
    if (s['page-size']) localStorage.setItem('page-size', String(s['page-size']));
  }
}
...
function render() {
  ...
  if (show) {
    ...
    d.style.cssText = `background:var(--float-panel-bg);border-radius:var(--radius-large);width:400px;max-width:calc(100vw - 32px);padding:24px;box-shadow:0 8px 40px rgba(0,0,0,0.15)`;
    ...
    d.querySelector('#lg-token')?.addEventListener('keyup', (e: any) => { tokenInput = e.target.value; if (e.key === 'Enter') handleLogin(); });
  }
}
</script>
